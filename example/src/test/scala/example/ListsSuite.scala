...
class ListsSuite extends FunSuite {
  ...
  test("sum of a list of zeroes") {
    assert(sum(List(0,0,0,0,0)) === 0)
  }
  
  test("sum of mixed negative and positive numbers") {
    assert(sum(List(0, -1, 2, -3, 4, -5, 7)) === 4)
  }
  ...
  test("max of mixed negative and positive numbers") {
    assert(max(List(0, 3, -5, 4)) === 4)
  }
  
  test("max of empty list") {
    try {
      max(List())
      assert(false) // Should throw an exception before this
    } catch {
  ...
  test("max of a repeated value") {
    assert(max(List(2, 2, 2, 2, 2)) === 2)
  }
  
  test("max of a repeated value with other values") {
    assert(max(List(2, -1, 2, 2, -5, 2, 1)) === 2)
  }
}
